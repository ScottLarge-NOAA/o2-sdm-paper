...
haul <- catch2$haul
colnames(haul) <- tolower(colnames(haul))
haul <- haul[,c("hauljoin","start_latitude", "start_longitude", "start_time")]
...
##No event id, and latitude/longitudes are not exact. So need to use nearest neighbor matching, and then exclude more than some set distance away
#Initialize
  test_day <- afsc2[1,]
  day <- unique(test_day$date)
  haul.2.use <- filter(haul, date==day)
  test <- RANN::nn2(haul.2.use[, c('Y', 'X')], test_day[, c('Y', "X")],k = 1)
  hauljoins<- haul.2.use[c(test$nn.idx),c("hauljoin")]
  test_day$distance <- test$nn.dists
  dats <- test_day
  dats$hauljoin <- hauljoins

for(k in 2:nrow(afsc2)){
  test_day <- afsc2[k,]
  day <- unique(test_day$date)
  haul.2.use <- filter(haul, date==day)
  test <- RANN::nn2(haul.2.use[, c('Y', 'X')], test_day[, c('Y', "X")],k = 1)
  ...
  dats$hauljoin <- hauljoins
  test_day$distance <- test$nn.dists
  dats <- bind_rows(dats, test_day)
}

#Remove points that are not actually synoptic
dats$hauljoin <- ifelse(dats$distance<1, dats$hauljoin, NA)
afsc2 <- dats
afsc2$distance <- NULL
