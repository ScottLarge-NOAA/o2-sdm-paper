...
saveRDS(test, file="outputs/o2_models_glorys/glorys_predictions_independent.rds")
...
ggplot(filter(test, est>0), aes(x=o2, y=est))+
  ...
  xlab("Observed dissolved oxygen")+
  ylab("GOBH predicted dissolved oxygen")

ggsave(
  paste("outputs/plots/glorys_independent_predsobs.png"),
  plot = last_plot(),
  device = NULL,
  path = NULL,
  scale = 1,
